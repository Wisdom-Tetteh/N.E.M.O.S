 ...
 * @author admin
 */
public class Efficiency_Link_Config {

    topologyreader tr = new topologyreader();
    int int_efficiancy2 = 2;
    int int_efficiancy1 = 1;
    double delays = 0;

    public Efficiency_Link_Config() {
        tr.freader(Efficiency_Start.topologyname);
        if (tr.linkname.size() > 0) {
            delays = Double.parseDouble("" + tr.linkdelay.get(0));
        }
    }
    ...
    public int getLinkEfficiancy(int speed) {
        if (speed == 57344) {
            return (int_efficiancy1);
        }
        if (speed == 131000) {
            return (int_efficiancy2);
        }
        return 0;
    }
