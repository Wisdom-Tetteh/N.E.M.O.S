 ...
 * @author admin
 */
public class Efficiency_RoutingInformationProtocol extends JFrame {
    ...
    public Efficiency_RoutingInformationProtocol(String psource) {
        source = psource;
        ...
        imageicon = toolkit.getImage("Mycomp.gif");
    ...
    public double getRIPEfficiancy(Efficiency_Packet p) {
        ...
        if ((router.getRouterState("" + p.getPacketDestination())).equals("Down")) {
            ...
            try {
                Efficiency_Link_Config l = new Efficiency_Link_Config();
                Efficiency_Router router = new Efficiency_Router();
                LinkedList temp1 = router.getNextHop("" + p.getPacketSource(), router.getRouterState("" + p.getPacketSource()));
                cost = new LinkedList();
                for (int li = 0; li < temp1.size(); li++) {
                    if ((router.getRouterState("" + temp1.get(li))).equals("Up")) {
                        LinkedList ln1 = router.getNextHop("" + temp1.get(li), router.getRouterState("" + temp1.get(li)));
                        ln1 = l.findRemove(ln1, "" + p.getPacketSource());
                        int ert = ln1.size();
                        LinkedList nodell1 = new LinkedList();
                        nodell1.add("" + p.getPacketSource());
                        if (("" + p.getPacketDestination()).equals("" + temp1.get(li))) {
                            if ((router.getRouterState("" + p.getPacketDestination())).equals("Up")) {
                                cost.add("" + (nodell1.size()));
                            }
                        }
                        ...
                        if (!("" + p.getPacketDestination()).equals("" + temp1.get(li))) {
                            for (int i = 0; i < ln1.size(); i++) {
                                if ((router.getRouterState("" + ln1.get(i))).equals("Up")) {
                                    if (("" + p.getPacketDestination()).equals("" + ln1.get(i))) {
                                        nodell1.add("" + ln1.get(i));
                                        break;
                                    } else {
                                        nodell1.add("" + ln1.get(i));
                                        LinkedList lt1 = router.getNextHop("" + ln1.get(i), router.getRouterState("" + ln1.get(i)));
                                        for (int j = 0; j < lt1.size(); j++) {
                                            if ((router.getRouterState("" + lt1.get(j))).equals("Up")) {
                                                ln1.add("" + lt1.get(j));
                                            }
                                        }
                                        ln1 = l.findRemove(ln1, source);
